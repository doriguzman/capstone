    ...
    axios
      ...
      .then(() => {
        ...
        this.state.allUsers.map(user => {
          const foundThread = this.findThreadByUsername(
            this.state.threads,
            user.username
          );
      ...
      .catch(err =>
        console.log("Error retrieving all threads in UserProfileCards.")
      );
  };
      ...
      if (tripObj.start_date) {
        ...
        const overlapPoints = numDaysOverlap => {
          let pts =
            ...
            msToDays(
              parseInt(
                new Date(myTrips.end_date) - new Date(myTrips.start_date)
              )
            ) *
            24;
